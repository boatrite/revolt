...
class ChunkBoundariesRenderer : public Renderer {
 ...
 public:
  ChunkBoundariesRenderer(std::shared_ptr<UIContext> ui_context_ptr,
                          std::shared_ptr<Camera> camera_ptr) :
    m_ui_context_ptr {ui_context_ptr},
    m_camera_ptr {camera_ptr} {
    m_ui_context_ptr->addKeyPressedHandler(GLFW_KEY_F3, this, [=]() { m_show = !m_show; });
    ...
    add_line(start + Chunk::CHUNK_SIZE_IN_UNIT_BLOCKS * x_hat,
             end + Chunk::CHUNK_SIZE_IN_UNIT_BLOCKS * x_hat,
             color);
    add_line(start + Chunk::CHUNK_SIZE_IN_UNIT_BLOCKS * z_hat,
             end + Chunk::CHUNK_SIZE_IN_UNIT_BLOCKS * z_hat,
             color);
    add_line(
      start + Chunk::CHUNK_SIZE_IN_UNIT_BLOCKS * x_hat + Chunk::CHUNK_SIZE_IN_UNIT_BLOCKS * z_hat,
      end + Chunk::CHUNK_SIZE_IN_UNIT_BLOCKS * x_hat + Chunk::CHUNK_SIZE_IN_UNIT_BLOCKS * z_hat,
      color);
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 static_cast<float>(m_vertices.size()) * sizeof(float),
                 m_vertices.data(),
                 GL_STATIC_DRAW);
